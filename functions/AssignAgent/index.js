...
module.exports = async function (context, req) {
  try {
    const categorie = (req.body && req.body.categorie) || req.query.categorie;
    const priorite  = (req.body && req.body.priorite)  || req.query.priorite || 'normal';
    ...
    const query = [
      ...
      "  AND t.statut IN ('ouvert','en_cours','en_attente')",
    ...
    if (!result.recordset.length) {
      context.res = {
        status: 404,
        body: { error: 'Aucun agent disponible', agent_id: null }
      };
      return;
    }
    ...
    context.res = {
      ...
      body: {
        ...
        niveau:         agent.niveau,
        tickets_actifs: agent.tickets_actifs,
        assigned_at:    new Date().toISOString()
      }
    };

  } catch (e) {
    context.log.error('AssignAgent erreur:', e.message);
    context.res = {
      status: 500,
